...
class FileService:
    ...
    FILE_CONFIG = {
        FileTypeEnum.DOCUMENT: {
            ...
            "max_size_mb": 50,  # 50 Mo
            "storage_path": settings.DOCUMENTS_STORAGE_PATH,
            "url_prefix": settings.DOCUMENTS_STORAGE_PATH,
        },
        FileTypeEnum.IMAGE: {
            "extensions": {".jpg", ".jpeg", ".png"},
            "max_size_mb": 20,   # 20 Mo
            "storage_path": settings.IMAGES_STORAGE_PATH,
            "url_prefix": settings.IMAGES_STORAGE_PATH,
        },
        FileTypeEnum.AUDIO: {
            "extensions": {".mp3", ".wav", ".ogg"},
            "max_size_mb": 150,  # 150 Mo
            "storage_path": settings.AUDIOS_STORAGE_PATH,
            "url_prefix": settings.AUDIOS_STORAGE_PATH,
        },
        FileTypeEnum.VIDEO: {
            "extensions": {".mp4", ".avi", ".mkv"},
            "max_size_mb": 300,  # 300 Mo
            "storage_path": settings.VIDEOS_STORAGE_PATH,  
            "url_prefix": settings.VIDEOS_STORAGE_PATH,
        }
    }
    ...
    def _ensure_directories(self) -> None:
        ...
        for file_type, config in self.FILE_CONFIG.items():
            ...
            try:
                ...
                logger.error(f"Erreur lors de la création du répertoire {storage_path} pour {file_type.value}: {str(e)}")
                raise HTTPException(
                    status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                    detail=f"Erreur lors de la création du répertoire {storage_path} pour {file_type.value}"
                )
    ...
    def _validate_file(self, file: UploadFile, file_type: FileTypeEnum) -> None:
        ...
        if file_ext not in config["extensions"]:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail=f"Extension {file_ext} non autorisée pour {file_type.value}. Extensions autorisées : {', '.join(config['extensions'])}"
            )
    ...
    async def upload_file(self, file: UploadFile, file_type: FileTypeEnum) -> str:
        """Upload un fichier unique et retourne son URL."""
        ...
        try:
            ...
            logger.info(f"Fichier {unique_filename} sauvegardé dans {storage_path} pour {file_type.value}")
            
            # Génération de l'URL
            url = f"{settings.SERVER_URL}{config['url_prefix']}/{unique_filename}"
            return url
        except Exception as e:
            logger.error(f"Erreur lors de l'upload du fichier {file.filename} pour {file_type.value}: {str(e)}")
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
    ...
    async def upload_files(self, files: List[UploadFile], file_type: FileTypeEnum) -> List[str]:
        """Upload plusieurs fichiers et retourne une liste d'URLs."""
        if not files:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
        ...
        urls = []
        for file in files:
            url = await self.upload_file(file, file_type)
            urls.append(url)
    ...
    async def delete_file(self, file_url: str, file_type: FileTypeEnum) -> str:
        ...
        config = self.FILE_CONFIG[file_type]
        file_path = Path(config["storage_path"]) / file_url.split("/")[-1]
        
        try:
            if file_path.exists():
                file_path.unlink()
                logger.info(f"Fichier {file_path} supprimé avec succès pour {file_type.value}.")
                return f"Fichier {file_url} supprimé avec succès"
            else:
                logger.warning(f"Fichier {file_path} non trouvé pour suppression pour {file_type.value}.")
                return f"Fichier {file_url} non trouvé"
        except Exception as e:
            logger.error(f"Erreur lors de la suppression du fichier {file_url} pour {file_type.value}: {str(e)}")
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                detail=f"Erreur lors de la suppression du fichier {file_url} pour {file_type.value}"
            )
