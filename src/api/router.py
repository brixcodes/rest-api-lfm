...
@router.get(
    ...
    description="Récupère une liste paginée de toutes les actualités avec les mêmes champs que le détail."
)
async def list_actualites(skip: int = 0, limit: int = 100, db: AsyncSession = Depends(get_async_db)):
    """
    Liste toutes les actualités avec pagination (même schéma que GET /actualites/{id}).

    - **skip**: Nombre d'actualités à sauter (défaut: 0).
    - **limit**: Nombre maximum d'actualités à retourner (défaut: 100).
    - **Réponses**:
        - **200**: Liste des actualités complètes (ex. `[{"id": 1, ...}]`).
        - **500**: Erreur interne du serveur.
    """
